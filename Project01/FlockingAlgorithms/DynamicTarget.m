...
vid = VideoWriter('dynamic-flocking.avi');
vid.FrameRate = 15;
...
for iteration = 1:nFrames
    ...
    COM(iteration, :) = mean(positions);
    
    %setting position of gamma agent (sensor node target) at current iteration
    gammaAgent.q(iteration, :) = [50+50*iteration/(n*2), 
    							  250-100*sin(iteration/(n*2))];
    							  
    %setting velocity of gamma agent
    if iteration > 1
        gammaAgent.p(iteration, :) = (gammaAgent.q(iteration, :) - gammaAgent.q(iteration - 1, :))/delta_t;
    else
        gammaAgent.p(iteration, :) = zeros(1,2);
    end
    
    %plotting the gamma agent
    plot(gammaAgent.q(:,1), gammaAgent.q(:,2), 'go', 'LineWidth', 2, 
    'MarkerEdgeColor', 'g', 'MarkerFaceColor', 'g', 'MarkerSize', 4.2);
    hold on;
    
    %plotting each sensor node
    plotNodes(sensorNode, n);
    
    %plotting the boundaries
    hold on
    plot(0, 0, 'k.');
    plot(200 + gammaAgent.q(iteration, 1), 400, 'k.');
    hold off;
    ...
    connectivity(iteration) = rank(AdjMat)/n;
    ...
    for i = 1:n
        ...
        sensorNode(i).u = 30*sum(GBT) + 12*sum(CT) 
        				  - 1.1*(sensorNode(i).q - gammaAgent.q(iteration,:)) 
        				  - 2*(sensorNode(i).p - gammaAgent.p(iteration, :));
...
plotTrajectory(sensorNode, nFrames, n); %plotting trajectory of all nodes
...
plotMeanAndTargetTrajectory(COM, gammaAgent, nFrames); %plotting trajectory of the 
													   %center of mass as well as the 
													   %gamma agent
...
function plotMeanAndTargetTrajectory(COM, gammaAgent, nFrames)
    
    figure
    for j = 1:nFrames
       hold on;
       plot(COM(j, 1), COM(j, 2), 'k.');
       plot(gammaAgent.q(j, 1), gammaAgent.q(j, 2), 'r.');
       hold off;
       if j ~= 1
           xc = [COM(j-1, 1) COM(j, 1)];
           xg = [gammaAgent.q(j-1, 1) gammaAgent.q(j, 1)];
           yc = [COM(j-1, 2) COM(j, 2)];
           yg = [gammaAgent.q(j-1, 2) gammaAgent.q(j, 2)];
           hold on;
           line(xc,yc, 'Color', 'k');
           line(xg, yg, 'Color', 'r');
           hold off;
